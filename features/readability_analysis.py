...
class Readability(Feature):
    ...
    def readability_analysis(self, text):
        ...
        # print("sentences per 100 words ", (100/avg_word_per_sentence))
        # print("average letter for 100 word: ",avg_letter_per_word*100)
        # print("word per sent: ",avg_word_per_sentence)
        ...
        if total_words != 0 and total_sentences != 0:
            dale_chall_index = 0.1579 * ((total_difficult_words / total_words) * 100) + 0.0496 * (total_words / total_sentences)
            dale_chall_index_mapped = dale_chall_index / 10
        else:
            dale_chall_index_mapped = 0.0

        if total_sentences != 0 and total_words != 0:
            flesch_kincaid_index = 206.835 - 1.015 * (total_words / total_sentences) - 84.6 * (total_syllables / total_words)
            flesch_kincaid_index_mapped = ((flesch_kincaid_index-0)/(100-0)*(1))
        else:
            flesch_kincaid_index_mapped = 0.0

        # print("dale_chall_index: ",dale_chall_index_mapped)
        # print("Coleman Liau: ", coleman_liau_index_mapped)
        # print("flesch kincaid: ", flesch_kincaid_index_mapped)
    ...
    def train(self, dale_chall_easy_words_path='/data/train_data/DaleChallEasyWordList.txt'):
        ...
        #print(self.dale_chall_easy_words)
